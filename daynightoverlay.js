...
*/
var DayNightOverlay = function(map, date, fillColor, id) {
    this.canvas_ = null;
    ...
    if ("undefined" != typeof date)
        this.date_ = date;
    "undefined" != typeof map && this.setMap(map)
};
...
DayNightOverlay.prototype.draw = function() {
    var a = this.getProjection(),
        b = this.getWorldDimensions_(a),
        a = this.getVisibleDimensions_(a, 250);
    ...
    var b = this.createTerminatorFunc_(a,
        b),
        d = this.isNorthernSun_(this.date_ ? this.date_ : new Date);
...
DayNightOverlay.prototype.getWorldDimensions_ = function(a) {
    var b = a.fromLatLngToDivPixel(DayNightOverlay.NORTH_),
        c = a.fromLatLngToDivPixel(DayNightOverlay.SOUTH_),
        a = a.getWorldWidth();
    return {
        x: b.x - a / 2,
        y: b.y,
...
DayNightOverlay.prototype.getVisibleDimensions_ = function(a, b) {
    var c = a.fromLatLngToDivPixel(this.getMap().getBounds().getNorthEast()),
        d = a.fromLatLngToDivPixel(this.getMap().getBounds().getSouthWest());
    return {
        x: d.x - b,
        y: c.y - b,
        width: c.x - d.x + 2 * b,
...
DayNightOverlay.prototype.createTerminatorFunc_ = function(a, b) {
    var c = this.date_ ? this.date_ : new Date,
        d = 2 * Math.PI,
        e = b.height / 2,
        f = a.height,
        g = a.x - b.x,
        ...
        i = d / b.width,
        j = b.height / Math.PI,
        k = (3600 * c.getUTCHours() + 60 * c.getUTCMinutes() + c.getUTCSeconds() + 43200) * (d / 86400),
        l = this.getDayOfYear_(c),
        c = this.getDayOfYear_(new Date(Date.UTC(c.getFullYear(), 2, 20))),
        m = 23.44 * Math.PI / 90,
        n = Math.sin(d * (l - c) / 365) * m;
    return function(a) {
...
DayNightOverlay.prototype.isNorthernSun_ = function(a) {
    var b = new Date(Date.UTC(a.getFullYear(), 2, 19)),
        c = new Date(Date.UTC(a.getFullYear(), 8, 18));
